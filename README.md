...
- Account (wallet) CRUD operations
...
6. Account Service validates the HMAC request and updates the transaction to `COMPLETED` or `FAILED`.
...
| Method | Path | Description |
|--------|------|-------------|
| PATCH | `/api/internal/transactions/{id}/` | Update transaction status |
